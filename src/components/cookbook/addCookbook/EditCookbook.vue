	...
	methods: {
		...
		editRecipe() {
			...
			let data = qs.stringify({
				...
				recipePrice: parseInt(this.cookbook.recipePrice * 100),
			...
			this.Axios(
				...
				result => {
					if (result.data.code === 200) {
						...
						// result.data.data.cateId = result.data.data.cateId.split("");
						result.data.data.step = JSON.parse(result.data.data.step);
						result.data.data.cookScript = JSON.parse(result.data.data.cookJson);
						result.data.data.state = JSON.stringify(result.data.data.state);
						this.cookbook = { ...result.data.data };
						let arr = new Array();
						arr.push(this.cookbook.cateId);
						this.cookbook.cateId = arr;
						this.cookbook.cateId = this.cookbook.cateId.map(Number);
						for (let i = 0; i < this.cookbook.step.length; i++) {
							if (
